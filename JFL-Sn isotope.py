...
Sn_initial = 16.2                     # Initial Sn concentration in the melt (ppm)
D_mineral_melt = 0.141                # Mineral–melt partition coefficient for Sn
D_fluid_melt = 7                      # Fluid–melt partition coefficient for Sn
P = 150                               # Pressure (MPa)
delta_Sn_initial = 1.208              # Initial Sn isotopic composition (δ units)
alpha1 = 0.9999                       # Fractionation factor: mineral–melt
alpha2 = 1.0001                       # Fractionation factor: fluid–melt

# Observational dataset
data = pd.DataFrame({
  "X_content": [26.10, 34.80, 25.10, 20.10, 22.40, 23.40, 24.50, 16.20,
                24.00, 20.10, 24.00, 16.40, 59.70, 54.10, 27.90],
  "Y_isotope": [0.938, 0.600, 0.478, 0.612, 0.940, 0.939, 0.946, 1.208,
                0.360, 0.592, 0.650, 0.817, 0.508, -0.244, 0.899]
})
...
# Effective bulk partition and isotopic fractionation parameters
...
def fit_func(X, a, X0, Y0):
    """
    Power-law fitting function for Sn–δ relationship
    """
...
# Fitted values and uncertainty
Y_fit = fit_func(data["X_content"], a_fit, X0, Y0)
error_std = np.std(data["Y_isotope"] - Y_fit)

X_grid = np.linspace(min(data["X_content"]),
                     max(data["X_content"]) + 10, 1000)
...
# Select top 100 best-fitting simulations
top100 = (
...
# --- 2. Three main Sn–δSn evolution curves --- #
...
plt.ylabel("δ124Sn (‰)")
plt.title("Combined Sn–δ124Sn Model: Rayleigh + Monte Carlo Simulation")
